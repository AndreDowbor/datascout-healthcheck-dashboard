...
class IMISClient:
    ...
    def _join_url(self, *parts):
        """
        Safely join URL parts, ensuring no double slashes occur.
        """
        if not parts:
            return ""
        
        # Start with the first part (usually base_url)
    ...
    def authenticate(self):
        ...
        token_url = self._join_url(self.base_url, "token")
    ...
    def fetch_iqa(
        ...
        base_url = self._join_url(self.base_url, "api", "IQA")
        request_params = {
            "QueryName": iqa_path,
        }
        if params:
        ...
        while has_next and (limit is None or offset < limit):
            ...
            if not result:
                raise Exception(f"🔴[IMISClient] Response processing failed at offset {offset:,}.")
    ...
    def _construct_path_request(
        ...
        """
        path_request = self._join_url(self.base_url, path.lstrip('/')) + f"&limit={page_size}&offset={offset}"
        if params:
            for param in params:
    ...
    def delete_member_property(
        ...
        """
        url = self._join_url(self.base_url, "api", table_name, f"~{member_id}|{ordinal}")
        try:
            ...
            print(f"🔴[IMISClient] Delete failed for member_id='{member_id}', ordinal={ordinal}: {e}")
            return False
    ...
    def add_member_property(
        ...
        Returns True if successful, False otherwise.
        """
        url = self._join_url(self.base_url, "api", table_name)
        ...
        try:
            ...
            if response.status_code >= 400:
                print(f"🔴[IMISClient] Add failed for '{property_name}', status: {response.status_code}.")
                return False
            return True
        except Exception as e:
            print(f"🔴[IMISClient] Add failed for '{property_name}': {e}")
            return False
    ...
    def list_iqas_in_folder(self, folder_path: str) -> List[Dict[str, Any]]:
        """
        Return a list of IQA document summaries (dicts) located in the specified iMIS folder.
    ...
    def list_documents_in_folder(self, folder_path: str) -> List[Dict[str, Any]]:
        """
        Return a list of document summaries (dicts) located in the specified iMIS folder.
        If the folder does not exist, return an empty list and log a warning.
        """
        document_id = self._get_document_id_from_path(folder_path)
        if not document_id:
    ...
    def _get_document_id_from_path(self, folder_path: str) -> str | None:
        ...
        """
        url = self._join_url(self.base_url, "api", "Document", "_execute")
        headers = {"Content-Type": "application/json"}
        body = {
            "$type": "Asi.Soa.Core.DataContracts.GenericExecuteRequest, Asi.Contracts",
            "OperationName": "FindByPath",
    ...
    def _get_folder_document_summaries(self, document_id: str) -> Dict[str, Any]:
        ...
        """
        url = self._join_url(self.base_url, "api", "DocumentSummary", "_execute")
        headers = {"Content-Type": "application/json"}
        body = {
            "$type": "Asi.Soa.Core.DataContracts.GenericExecuteRequest, Asi.Contracts",
            "OperationName": "FindDescendantDocumentsInFolder",
